...
from py_space_zc import irf_time, get_cdf_var, year_month_day, tint_data, ts_scalar, ts_vec_xyz, ts_spectr, ts_skymap
...
def get_data(tint, var):
   ...
   #        tint=['2018-11-01T01:00:00','2018-11-01T02:00:00']
    ...
    #%% read the magnetic field (32Hz), mso
      ...
      # If loadmat fails, use h5py to open HDF5 MATLAB v7.3 files
                   with h5py.File(filename, 'r') as file:
                        mat_data = {key: np.array(file[key]) for key in file.keys()}
                        mat_data["time"] = np.transpose(mat_data["time"])
                        mat_data["Bmso"] = np.transpose(mat_data["Bmso"])
                        mat_data["Pmso"] = np.transpose(mat_data["Pmso"])
    ...
    #%% read the magnetic field (1Hz), MSO and MSE
        ...
        for filename in filelist:
            ...
            start = datetime.datetime.strptime(f"{parts[0]}_{parts[1]}", "%Y%m%d_%H%M%S")
            end = datetime.datetime.strptime(f"{parts[2]}_{parts[3].split('.')[0]}", "%Y%m%d_%H%M%S")
            file_start = np.datetime64(start)
            file_end = np.datetime64(end)

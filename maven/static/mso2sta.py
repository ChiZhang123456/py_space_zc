...
def mso2sta(inp, d1):
    """
    Convert a vector time series from MSO to STATIC coordinates using the
    per-time rotation matrix contained in STATIC D1 data.

    Parameters
    ----------
    inp : TSeries-like (vector, shape = [time, 3])
        Input vector in MSO coordinates. Must be a pyrfu/py_space_zc time series
    ...
    -------
    out : TSeries-like
        The same vector expressed in STATIC coordinates, on d1['time'] grid.
        out.attrs['Coordinates'] == 'STATIC'
    ...
    # Invert rotation by transpose: MSO -> STA
    R_mso2sta = np.swapaxes(R_sta2mso, 1, 2)           # shape: [N, 3, 3]

    # Batched matrix multiply: for each time step i, v_sta[i] = R_mso2sta[i] @ v_mso[i]
    v_sta = np.einsum('nij,nj->ni', R_mso2sta, inp_resampled)  # shape: [N, 3]

    # Wrap back to a vector time series with STATIC coord tag
    out = py_space_zc.ts_vec_xyz(t_target, v_sta,
                                 attrs={'Coordinates': 'STATIC'})
    return out
...
if __name__ == "__main__":
    ...
    xmso_sta = mso2sta(xmso, d1)
    ymso_sta = mso2sta(ymso, d1)
    zmso_sta = mso2sta(zmso, d1)

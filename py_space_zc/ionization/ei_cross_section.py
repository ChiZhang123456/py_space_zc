...
def read_txt_data(filename, from_x1e16=True):
    ...
    return {
        'energy': energy,                 # eV
        'elastic': _scale(elastic),       # cm^2
        'ionization': _scale(ionization), # cm^2
        'others': _scale(others),         # cm^2
    }
...
def ei_cross_section(energy, species):
    ...
    filename = os.path.join(script_dir, filename_only)
...
if __name__ == "__main__":
    ...
    ax.plot(energy, o2_ei, color='navy', label='O₂')
    # ax.plot(energy, co_ei, color='navy', label='CO')
    ax.plot(energy, co2_ei, color='purple', label='CO₂')

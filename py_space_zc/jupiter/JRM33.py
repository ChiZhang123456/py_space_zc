...
if __name__ == '__main__':
    ...
    x = np.linspace(-30.0, 10.0, num=1000)
    z = np.linspace(-10.0, 10.0, num=1000)
    XX, ZZ = np.meshgrid(x, z)
    YY = np.zeros_like(XX)
    X = XX.reshape(-1)
    Y = YY.reshape(-1)
    Z = ZZ.reshape(-1)
    res = GetJupiterMag(X, Y, Z, xyz_in=True, Bxyz_Out=True, CAN=True)
    Bx = res[:, 0].reshape(1000, 1000)
    Bz = res[:, 2].reshape(1000, 1000)
    Bt = res[:, 3].reshape(1000, 1000)
    ax, pcm, cbar = plot.plot_pcolor(None, x, z, Bt, cscale='log', clim=[1e0, 1e7])
    ax.streamplot(XX, ZZ, Bx, Bz, density=[1])
    plt.show()

...
def bs_mpb(ax, color=False):
    ...
    ax.fill(np.cos(theta), np.sin(theta), color='lightgray', edgecolor='lightgray', zorder=1, linewidth=0.3,)
    theta = np.linspace(np.pi / 2, 3 * np.pi / 2, 500)
    ax.fill(np.cos(theta), np.sin(theta), color='darkgray', edgecolor='darkgray', zorder=1, linewidth=0.3,)

...
def show_overview(tint):
    ...
    _, s1, c1 = plot.scatter_time(ax_left[0,0], Pmvn[:, 0], Rmvn, t_mvn,
                                  cmap='Spectral_r', size=5.0, min_nticks=3)
    maven.bs_mpb(ax_left[0,0])  # Add bow shock & MPB model
    plot.set_axis(
        ax_left[0,0], xlim=(-4.0, 2.0), ylim=(0.0, 5.5),
    ...
    c1.ax.set_ylabel('')
    plot.adjust_colorbar(ax_left[0,0], c1, 0.005, 0.8, 0.015)

    # --- Bottom left: Y–Z (with Mars as black disk)
    _, s1, c2 = plot.scatter_time(ax_left[1,0], Pmvn[:, 1], Pmvn[:, 2], t_mvn,
                                  cmap='Spectral_r', size=5.0, min_nticks=3)
    ax_left[1,0].add_patch(patches.Circle((0, 0), radius=1, color='black', alpha=0.45, zorder=10))
    plot.set_axis(
    ...
    c2.ax.set_ylabel('')
    plot.adjust_colorbar(ax_left[1,0], c2, 0.005, 0.8, 0.015)
    ...
    ax, cax = maven.plot_swea_omni(ax_right[4,0], tint, clim=[1e5, 5e8])

    # --- Panel 6: MAVEN magnetic field vector & magnitude
    maven.plot_B(ax_right[5,0], tint)

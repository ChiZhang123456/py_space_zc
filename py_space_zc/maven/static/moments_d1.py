import numpy as np
from py_space_zc import maven, vdf, ts_vec_xyz, rotate_tensor
from pyrfu.pyrf import extend_tint, resample, normalize
...
def moments_d1(d1_raw, species, Emin = 0.0, Emax = 500000.0, 
    ...
    bdmpa = normalize(d1_raw['Bdmpa'])
    moments['T_fac'] = rotate_tensor(moments['T'], "fac", bdmpa, "qq")
    moments['P_fac'] = rotate_tensor(moments['P'], "fac", bdmpa, "qq")
    moments['P2_fac'] = rotate_tensor(moments['P2'], "fac", bdmpa, "qq")
    moments['Pth_avg'] = (moments['P_fac'][:,0,0] +
                          moments['P_fac'][:,1,1] +
                          moments['P_fac'][:,2,2])/3
    moments['T_avg'] = (moments['T_fac'][:,0,0] +
                          moments['T_fac'][:,1,1] +
                          moments['T_fac'][:,2,2])/3

...
import copy
import py_space_zc
from py_space_zc import normalize, cross, ts_tensor_xyz, ts_vec_xyz, time_eval
...
def reduced_d1_1d(
    d1: dict,
    mso_axis: Union[Iterable[float], np.ndarray],
    species: str,
    ...
    """
    Reduce a 3D ion velocity distribution (MAVEN STATIC D1) onto a 1D
    ...
    mso_axis: array-like
         vectors in MSO. Shapes allowed: (3,), (1,3), or (nt,3).
    species : str
    ...
    Returns
    -------
    f1D : py_space_zc.vdf-compatible 1D reduced distribution
    """
    ...
    t_arr = d1_corr["time"]
    ax_ts = _tile_to_ts_vec(t_arr, np.asarray(mso_axis), "mso_axis")

    # convert the two MSO axes to STA frame (STATIC native) time-by-time
    # so the reduction basis matches the native VDF frame
    ax_sta = py_space_zc.maven.static.mso2sta(ax_ts, d1_corr)  # (nt,3)
    ax2_sta = np.ones_like(ax_sta)   # [1, 1, 1]

    # build right-handed orthonormal triad in STA
    e1, e2, e3 = _ensure_orthonormal_triad(ax_sta, ax2_sta)
    ...
    # velocity grid (m/s)
    if vg is None:
        vg = np.linspace(-500.0, 500.0, 200) * 1e3  # km/s → m/s
    else:
        vg = np.asarray(vg, dtype=float) * 1e3   # km/s → m/s
    ...
    if correct_vsc:
        ...
        vsc_axis = py_space_zc.dot(vsc_mso, e1)
        vsc_axis = np.nanmean(vsc_axis, axis = 0)
    else:
    ...
    f1D = f1D.assign_coords(vx=f1D.coords['vx'] + float(vsc_axis),)

...
from pyrfu.pyrf import mva, new_xyz, norm, time_clip
...
class MvaGui:
    ...
    def init_fig(b_xyz):
        ...
        plot_line(ax1, b_xyz)
        plot_line(ax1, norm(b_xyz), color="gray")
        ax1.set_xlim(b_xyz.time.data[0], b_xyz.time.data[-1])
        ax1.set_ylabel("$B$ [nT]")
    ...
    def update_onselect(self, tmin, tmax):
        ...
        if len(region_t) >= 2:
            self.update_fig(b_xyz_clip, tmin, tmax)
    ...
    def update_fig(self, b_xyz_clip, tmin, tmax):
        ...
        plot_line(ax2, b_lmn_clip)
        plot_line(ax2, norm(b_lmn_clip), color="gray")

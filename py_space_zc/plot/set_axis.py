...
def set_axis(
    ...
    formatter_fontsize: Optional[int] = None,
    ...
    # Force formatter-applied tick label fontsize (for DateFormatter etc.)
    # ConciseDateFormatter exception handler
    if formatter_fontsize is not None:
        # Try to handle mdates.ConciseDateFormatter
        fmt = ax.xaxis.get_major_formatter()
        if hasattr(fmt, "fontsize"):  # specifically for ConciseDateFormatter
            fmt.fontsize = formatter_fontsize
        # Also try to apply fallback to visible labels

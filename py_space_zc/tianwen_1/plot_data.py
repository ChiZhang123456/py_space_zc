...
def plot_B(ax, tint):
    ...
    tint_new = extend_tint(tint, [-5.0, 5.0])
    B = tw.get_data(tint_new, "B")
    plot_line(ax, B["Bmso"])
...
def plot_minpa_mod1_omni(ax, tint, species, interp=False , clim=None):
    """
    ...
    # Get data
    tint_new = extend_tint(tint, [-20.0, 20.0])
    data = tw.minpa_omni(tint_new, species, interp = interp)

...
def plot_maven_tianwen_xr(
    ...
    """
    Plot MAVEN & Tianwen-1 spacecraft trajectories in X–R (MSO) coordinates,
    ...
    _, s1, c1 = plot.scatter_time(ax, Pmvn[:, 0], Rmvn, t_mvn,
                                  cmap=cmap, size=size, min_nticks=3)
    _, s2, c2 = plot.scatter_time(ax, Ptw[:, 0], Rtw,  t_tw,
                                  cmap=cmap, size=size, min_nticks=3)
    ...
    ax.scatter(Pmvn[0, 0] + sat_offset[0], Rmvn[0] + sat_offset[1],
               marker="*", s=5*size, c='black',
               edgecolors="black", linewidths=1.2, zorder=10)
    ax.scatter(Ptw[0, 0] + sat_offset[0], Rtw[0] + sat_offset[1],
    ...
    ax.set_xlabel(r"$X_{\mathrm{MSO}}$ (R$_\mathrm{M}$)")
    ax.set_ylabel(r"$\sqrt{Y_{\mathrm{MSO}}^2 + Z_{\mathrm{MSO}}^2}$ (R$_\mathrm{M}$)")
...
if __name__ == "__main__":
    ...
    ax, cbar, leg = plot_maven_tianwen_xr(

...
def show_maven_tianwen_data(tint):
    ...
    # X–R panel
    _, cbar_xr, _ = tianwen_1.plot_maven_tianwen_xr(ax_left[0,0], tint)
    plot.set_axis(
        ax_left[0,0], xlim=(-4.0, 4.0), ylim=(0.0, 6.0),
    ...
    ax_left[0,0].set_aspect('equal')
    cbar_xr.ax.set_ylabel('')
    pos = ax_left[0,0].get_position()
    cbar_xr.ax.set_position([pos.x1 + 0.005, pos.y0 + 0.01, 0.015, pos.height - 0.02])

    # Y–Z panel
    _, cbar_yz, _ = tianwen_1.plot_maven_tianwen_yz(ax_left[1,0], tint)
    plot.set_axis(
        ax_left[1,0], xlim=(-4.0, 4.0), ylim=(-4.0, 4.0),
        tick_fontsize=12, label_fontsize=14,
    ...
    # STATIC C6: H
    ax_h, cax_h = maven.plot_sta_c6(ax_right[0,0], tint, "H", correct_background=False)
    ...
    # STATIC C6: O
    ax_o, cax_o = maven.plot_sta_c6(ax_right[1,0], tint, "O", correct_background=False)
    ...
    ax_o2, cax_o2 = maven.plot_sta_c6(ax_right[2,0], tint, "O2", correct_background=False)
    ...
    # SWIA omni
    ax_swia, cax_swia = maven.plot_swia_omni(ax_right[3,0], tint)
    ...
    maven.plot_B(ax_right[4,0], tint)
    B = maven.get_data(tint,'B')
    Bt = ts_scalar(B["Bmso"].time.data, norm(B["Bmso"].data).reshape(-1))
    plot.plot_line(ax_right[4,0], Bt, color = "grey", linewidth = 1.0,)
    bmax = np.max(Bt.data)
    ax_right[4,0].legend(["$B_x$", "$B_y$", "$B_z$", r"$|\mathbf{B}|$"],
                         loc="center right", bbox_to_anchor=(1.123, 0.5),
                         frameon=False, fontsize=12, handlelength=0.5, ncol=1)
    
    plot.add_text(ax_right[4,0], 'MAVEN', 0.99, 0.99, color='white', facecolor='grey')
    if bmax>50:
        ax_right[4,0].set_ylim(-50.0, 50.0)
    
    

    # Tianwen-1 magnetic field
    tianwen_1.plot_B(ax_right[5,0], tint)
    plot.add_text(ax_right[5,0], 'Tianwen-1', 0.99, 0.99, color='white', facecolor='grey')
    B = tianwen_1.get_data(tint,'B')
    bmax = np.max(norm(B["Bmso"].data))
    Bt = ts_scalar(B["Bmso"].time.data, norm(B["Bmso"].data).reshape(-1))
    plot.plot_line(ax_right[5,0], Bt, color = "grey", linewidth = 1.0,)
    if bmax>50:
        ax_right[5,0].set_ylim(-50.0, 50.0)
        
    # Clock angle (dot style)
    tianwen_1.plot_maven_tianwen_clock_angle(ax_right[6,0], tint, "dot", markersize=50, linewidth=3)

    for ax in ax_right[:,0]:

...
def plasma_moments(PSD, energymat, dEmat, phimat, thetamat, dphimat, dthetamat, species):
    """
    Compute plasma moments (density, velocity, pressure, temperature, energy fluxes)
    from a 4D phase space density (PSD) array.

    Parameters
    ----------
    PSD : ndarray
        Phase space density [s^3/m^6], shape = (ntime, nenergy, nphi, ntheta)

    energymat : ndarray
        Particle energy in electronvolts [eV], typically already corrected by spacecraft potential.
        Shape = (ntime, nenergy, nphi, ntheta)

    dEmat : ndarray
        Differential energy width [eV], corresponding to energy bin width.
        Shape = (ntime, nenergy, nphi, ntheta)

    phimat : ndarray
        Azimuthal angle φ in degrees [°], to be converted to radians internally.
        Shape = (ntime, nenergy, nphi, ntheta)

    thetamat : ndarray
        Elevation (or polar) angle θ in degrees [°], to be converted to radians internally.
        Shape = (ntime, nenergy, nphi, ntheta)

    dphimat : ndarray
        Azimuth angle bin width [°], to be converted to radians.
        Shape = (ntime, nenergy, nphi, ntheta)

    dthetamat : ndarray
        Polar angle bin width [°], to be converted to radians.
        Shape = (ntime, nenergy, nphi, ntheta)

    species : str
        Particle species string, e.g., 'H+', 'O+', 'He+'.
        Used to determine particle mass and charge.

    Returns
    -------
    dict
        Plasma moment quantities in physical units:
        - 'n'   : number density [cm⁻³]
    ...
    Notes
    -----
    - Uses cartesian conversion of velocities from spherical grid.
    - Only valid for non-relativistic, gyrotropic distribution functions.
    - Assumes full 4π coverage or appropriate fill value handling.
    """
    num_time = PSD.shape[0]
    kb = constants.Boltzmann  # Boltzmann constant [J/K]

    # === 1. Get particle mass and charge based on species name ===
    ...
    # === 8. Second moment: total momentum flux tensor P_ij [Pa] ===
    ...
    # === 9. Dynamic pressure: n m V_i V_j ===
    ...
    T_psd = np.where(n_psd[:, None, None] != 0, P_psd / (n_psd[:, None, None] * kb), np.nan)
    T_psd = T_psd * kb / qe  # convert from K to eV
    ...
    # === 14. Enthalpy flux: H_i = (trace(P)/2) * V_i + V_j * P_ij ===

...
def reduce(vdf, xyz, dim: str = "1d", base: str = "cart", **kwargs):
    ...
    **kwargs
    ...
    for i_t in tqdm.tqdm(range(n_t), ncols=60):  # display progress
        ...
        e_min_idx = np.where(energy > thresh_e)[0][0]
        f_3d[:e_min_idx] = 0.0
        ...
        if theta.ndim == 1:
            ...
            theta = theta[i_t, :]
        elif theta.ndim == 3 and theta.shape[0] == n_t and theta.shape[1] == len(energy):
            # Case: time and energy-dependent 3D theta [time, energy, theta]
            theta = theta[i_t, :, :]

        # Convert from instrument elevation (0°–180°) to pitch angle centered [-90°–90°]
